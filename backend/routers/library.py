...
def _movie_from_joined(d: dict) -> dict | None:
    if not d.get("m_id"):
        return None
    return {
        "id": d["m_id"],
...
def _entry_from_joined(d: dict) -> dict:
    return {
        "id": d["id"],
        "user_id": d["user_id"],
        "movie_id": d["movie_id"],
...
async def get_library(
    ...
    for row in entry_rows:
        entry = _entry_from_joined(dict(row))
        (to_watch if entry["list_type"] == "to_watch" else watched).append(entry)

    return {"liked": liked, "to_watch": to_watch, "watched": watched}


@router.post("/entries", response_model=LibraryEntryResponse, status_code=201)
async def add_library_entry(
    ...
    if body.movie_id is not None:
        ...
        # Upsert if same (user, movie) already exists
        async with db.execute(
        ...
        if existing:
            ...
            await db.commit()
            return await _fetch_entry(existing["id"], db)
...
@router.patch("/entries/{entry_id}", response_model=LibraryEntryResponse)
async def update_library_entry(
    ...
    if updates:
        ...
        values = [*updates.values(), entry_id]
...
async def _fetch_entry(entry_id: int, db) -> dict:
    async with db.execute(
        ...
        row = await cur.fetchone()
    return _entry_from_joined(dict(row))
